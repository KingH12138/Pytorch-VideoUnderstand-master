...
#         self.trans = VisionTransformer(img_size=4, patch_size=1, in_c=512, depth=4, num_classes=n_classes)
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        # self.fc = nn.Linear(8192, n_classes)
        # self.trans = VisionTransformer(img_size=4, patch_size=1, in_c=512 , depth=6, num_classes=n_classes)
        self.trans = MyTransformer(512, num_class=n_classes)
        # self.fc = nn.Linear(block_inplanes[3] * block.expansion, n_classes)
        self.soft = nn.Softmax(dim=1)
...
class MyTransformer(nn.Module):
    def __init__(self,inplanes,num_class=4):
        ...
        inputs:tensor(b,c,t) which is meaning h and w dim are pooled.
        """
