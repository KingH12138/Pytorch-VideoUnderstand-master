...
class ClassActionDataset(Dataset):
    def __init__(self, frame_dir, csv_path, resize):
        super(ClassActionDataset, self).__init__()
    ...
    def __getitem__(self, idx):
        frame_dir = self.df['path'][idx]
        label = self.df['label'][idx]    # 多个标签默认取第一个
        frame_list = []
...
def get_dataloader(frame_dir, csv_path, resize, batch_size, train_percent=0.9):
    dataset = ClassActionDataset(frame_dir, csv_path, resize)
